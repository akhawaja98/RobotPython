...
def neverEndingFunctions():
    def collisionDetection():
        ...
        while True:
            ...
            w.move(ball, XSpeed, YSpeed)
            if 730 >= Pos[3] or 740 <= Pos[1]: #bounces off landmark 1
                XSpeed = -XSpeed
            if 370 >= Pos[2] or 400 <= Pos[0]:
                YSpeed = -YSpeed
                
            w.move(ball, XSpeed, YSpeed)
            if 600 >= Pos[3] or 610 <= Pos[1]: #bounces off landmark 2
                XSpeed = -XSpeed
            if 50 >= Pos[2] or 40 <= Pos[0]:
                YSpeed = -YSpeed

            w.move(ball, XSpeed, YSpeed)
            if 450 >= Pos[3] or 460 <= Pos[1]: #bounces off landmark 3
                XSpeed = -XSpeed
            if 170 >= Pos[2] or 160 <= Pos[0]:
                YSpeed = -YSpeed

            w.move(ball, XSpeed, YSpeed)
            if 370 >= Pos[3] or 380 <= Pos[1]: #bounces off landmark 4
                XSpeed = -XSpeed
            if 550 >= Pos[2] or 540 <= Pos[0]:
                YSpeed = -YSpeed

            w.move(ball, XSpeed, YSpeed)
            if 990 >= Pos[3] or 1000 <= Pos[1]: #bounces off landmark 5
                XSpeed = -XSpeed
            if 430 >= Pos[2] or 400 <= Pos[0]:
                YSpeed = -YSpeed

            w.move(ball, XSpeed, YSpeed)
            if 1180 >= Pos[3] or 1200 <= Pos[1]: #bounces off landmark 6
                XSpeed = -XSpeed
            if 320 >= Pos[2] or 300 <= Pos[0]:
                YSpeed = -YSpeed
